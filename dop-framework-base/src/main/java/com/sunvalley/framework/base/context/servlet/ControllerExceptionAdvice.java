...
import static com.sunvalley.framework.core.exception.BusinessException.*;
...
public class ControllerExceptionAdvice implements InitializingBean {
	 ...
	 * 开启api调试
	 */
	@Value("${dop.apiError.debug:false}")
	private Boolean debugApiException = false;
	/**
	 * header.debug = eCh0
	 */
	@Value("${dop.apiError.header:eCh0}")
	private String debugHeaderValue = "eCh0";
	/**
	 * 开启rmi调试
	 */
	@Value("${dop.rmiError.debug:false}")
	private Boolean debugRmiException = false;
...
//		}
	...
	protected void fillDebug(HttpServletRequest request, Exception ex, Map<String, Object> values) {
		if (
			(Boolean.TRUE.equals(debugApiException) && debugHeaderValue.equals(request.getHeader("debug")))
				|| (Boolean.TRUE.equals(debugRmiException) && DopWebMvcConfigurer.isRmiRequest(request))
		) {
