package com.sunvalley.framework.cache.client;
 ...
 *  3、client统一使用了string泛型,bean转换是通过{@link com.sunvalley.framework.core.utils2.UtilJson}完成转换;
...
public class RedisOpsClient {
	...
	public <T> List<T> getList(String key, Class<T> elementClazz) {
		...
		return StringUtils.isEmpty(json) ? null : UtilJson.readList(json, elementClazz);
	}
	...
	public Long del0(Collection<String> keys) {
		if (UtilCollection.isEmpty(keys)) return 0L;
	...
	public void hmset(String key, Map<String, ?> objects) {
		assertTrue(verifyKeys(key) && !UtilCollection.isEmpty(objects), "key & value is not exist");
	...
	public <T> Set<T> smembers(String key, Class<T> elementClazz) {
		...
		if (!UtilCollection.isEmpty(members)) {
	...
	private static <T> T redisStrToValue(String object, Class<T> clazz) {
		if (clazz == String.class) {
			...
			return UtilJson.readValue(object, clazz);
		}
	}
	...
	private static String valueToRedisStr(Object obj) {
		if (obj == null) {
			...
			return UtilJson.toString(obj);
		}
	}

	private static boolean verifyKeys(String... keys) {
		return !UtilCollection.isEmpty(keys) && !StringUtils.isAnyBlank(keys);
	}

	private static boolean verifyValues(Object... values) {
		return !UtilCollection.isEmpty(values) && ObjectUtils.allNotNull(values);
	}
