...
public class LogInitializer implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    public void initialize(ConfigurableApplicationContext applicationContext) {
        ...
        log.info("launch before log---> {}", envLogLevel.getLaunchBefore());
        //日志文件配置
     ...
     * 刷新日志配置
     */
    private static void refreshLogConfig(Class<?> clazz) {
        try {
    ...
    public static void refreshLogConfig(String logLevel, Class<?> clazz) {
        Properties properties = System.getProperties();
        if (logLevel.toLowerCase().equals(properties.getProperty(LogConstants.ROOT_LOG_LEVEL).toLowerCase())) {
            return;
        }
        log.info("refreshLogConfig  -->", logLevel);
        properties.setProperty(LogConstants.ROOT_LOG_LEVEL, logLevel);
        refreshLogConfig(clazz);
    }
}
